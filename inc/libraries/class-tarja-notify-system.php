...
/**
 * Class tarja_Notify_System
 */
class tarja_Notify_System extends Epsilon_Notify_System {
	/**
	...
	public static function has_posts() {
		$has_posts = get_option( 'tarja_importer_finished' );
	...
	public static function widget_importer_exporter_title() {
		...
		if ( ! $installed ) {
			return __( 'Install: Widget Importer Exporter Plugin', 'tarja' );
		}

		$active = self::check_plugin_is_active( 'widget-importer-exporter' );
		if ( $installed && ! $active ) {
			return __( 'Activate: Widget Importer Exporter Plugin', 'tarja' );
		}
	...
	public static function wordpress_importer_title() {
		...
		if ( ! $installed ) {
			return __( 'Install: WordPress Importer', 'tarja' );
		}

		$active = self::check_plugin_is_active( 'wordpress-importer' );
		if ( $installed && ! $active ) {
			return __( 'Activate: WordPress Importer', 'tarja' );
		}
	...
	public static function wordpress_importer_description() {
		$installed = self::check_plugin_is_installed( 'wordpress-importer' );
		if ( ! $installed ) {
			return __( 'Please install the WordPress Importer to create the demo content.', 'tarja' );
		}

		$active = self::check_plugin_is_active( 'wordpress-importer' );
		if ( $installed && ! $active ) {
			return __( 'Please activate the WordPress Importer to create the demo content.', 'tarja' );
		}
	...
	public static function widget_importer_exporter_description() {
		$installed = self::check_plugin_is_installed( 'widget-importer-exporter' );
		if ( ! $installed ) {
			return __( 'Please install the WordPress widget importer to create the demo content', 'tarja' );
		}

		$active = self::check_plugin_is_active( 'widget-importer-exporter' );
		if ( $installed && ! $active ) {
			return __( 'Please activate the WordPress Widget Importer to create the demo content.', 'tarja' );
		}
