...
class tarja_Helper {
	...
	public static function customize_partial_blogname() {
		echo esc_html( get_option( 'blogname', 'tarja' ) );
	}
	...
	public static function add_breadcrumbs() {
		$breadcrumbs = new tarja_Breadcrumbs();
	...
	public static function categorized_blog() {
		$all_the_cool_cats = get_transient( 'tarja_categories' );
		if ( false === $all_the_cool_cats ) {
			...
			set_transient( 'tarja_categories', $all_the_cool_cats );
		}

		if ( $all_the_cool_cats > 1 ) {
			// This blog has more than 1 category so tarja_categorized_blog should return true.
			return true;
		} else {
			// This blog has only 1 category so tarja_categorized_blog should return false.
			return false;
		}
	}
	...
	public static function entry_footer() {
		$tags_list = get_the_tag_list( '', esc_html__( ' / ', 'tarja' ) );
		if ( ! empty( $tags_list ) ) {
			echo '<div class="entry-tags">' . esc_html__( 'TAGS: ', 'tarja' ) . $tags_list . '</div>';
		}
	}
