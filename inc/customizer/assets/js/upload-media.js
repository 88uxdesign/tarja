jQuery( function( $ ) {
  mediaControl = {
    ...
    select: function() {
      ...
      var selector = $( '.tarja-media-control' ).find( mediaControl.selector );
      ...
      var data = {
        action: [ 'tarja_Helper', 'get_attachment_image' ],
        args: { attachment_id: id }
      };

      $.ajax( {
        type: 'POST',
        data: { action: 'tarja_ajax_action', args: data },
        dataType: 'json',
        url: WPUrls.ajaxurl,
        success: function( data ) {
          $( mediaControl.container ).find( 'img' ).remove();
          $( mediaControl.container ).find( 'label' ).after( data.img );
          $( '.tarja-media-control' ).find( mediaControl.selector ).change();
        }
      } );
    ...
    init: function() {
      ...
      context.on( 'click', '.tarja-media-control > .upload-button',
          function( e ) {
            e.preventDefault();
      ...
      context.on( 'click', '.tarja-media-control > .remove-button',
          function( e ) {
            e.preventDefault();
            var container = $( this ).parent(),
                sibling = container.find( '.image-id' ),
