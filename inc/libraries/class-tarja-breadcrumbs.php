...
class tarja_Breadcrumbs {
	...
	public function __construct() {
		...
		$defaults = array(
			'home_prefix'            => get_theme_mod( 'tarja_blog_breadcrumb_menu_prefix', '' ),
			'separator'              => get_theme_mod( 'tarja_blog_breadcrumb_menu_separator', '/' ),
			'show_post_type_archive' => '1',
			'show_terms'             => get_theme_mod( 'tarja_blog_breadcrumb_menu_post_category', 1 ),
			'home_label'             => esc_html__( 'Home', 'tarja' ),
			'tag_archive_prefix'     => esc_html__( 'Tag:', 'tarja' ),
			'search_prefix'          => esc_html__( 'Search:', 'tarja' ),
			'error_prefix'           => esc_html__( '404 - Page not Found', 'tarja' ),
		);

		// Setup a filter for changeable variables and merge it with the defaults
		$args     = apply_filters( 'tarja_breadcrumbs_defaults', $defaults );
	...
	private function prepare_breadcrumb_html() {
		...
		if ( is_singular() ) {
			...
			if ( isset( $this->post->post_type ) && get_post_type_archive_link( false ) && $this->show_post_type_archive ) {
	...
	private function wrap_breadcrumbs() {
		$this->html_markup = sprintf( '<div class="tarja-breadcrumbs">%s</div>', $this->html_markup );
	}
	...
	private function get_breadcrumb_prefix() {
		...
		if ( ! is_front_page() ) {
			...
			if ( $this->home_prefix ) {
				$prefix = sprintf( '<span class="tarja-breadcrumb-prefix">%s:</span>', esc_html( $this->home_prefix ) );
			}
		}
	...
	private function get_single_breadcrumb_markup( $title, $link = '', $separator = true, $microdata = true ) {
		...
		if ( $separator ) {
			$separator_markup = sprintf( '<span class="tarja-breadcrumb-sep">%s</span>', $this->separator );
		}
