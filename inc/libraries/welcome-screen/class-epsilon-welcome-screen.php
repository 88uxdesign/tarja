...
class Epsilon_Welcome_Screen {
	...
	public function __construct( $config = array() ) {
		$defaults = array(
			'theme-name'  => '',
			'theme-slug'  => '',
			'author-logo' => get_template_directory_uri() . '/inc/libraries/welcome-screen/img/Logo-88UX.png',
			'actions'     => array(),
			'plugins'     => array(),
			//'notice'      => '',
			'sections'    => array(),
		...
		if ( isset( $_POST['args'], $_POST['args']['nonce'] ) && ! wp_verify_nonce( sanitize_key( $_POST['args']['nonce'] ), 'welcome_nonce' ) ) {
			wp_die(
				wp_json_encode(
					array(
						'status' => false,
						'error'  => esc_html__( 'Not allowed', 'tarja' ),
					)
				)
			);
		}
		...
		if ( ! class_exists( $args_action[0] ) ) {
			wp_die(
				wp_json_encode(
					array(
						'status' => false,
						'error'  => esc_html__( 'Class does not exist', 'tarja' ),
					)
				)
			);
		}
		...
		if ( is_admin() ) {
			...
			wp_localize_script(
				...
				array(
					...
					'no_required_actions_text' => esc_html__( 'Hooray! There are no required actions for you right now.', 'tarja' ),
	...
	private function add_admin_notice() {
		...
		if ( empty( $this->notice ) ) {
			...
			$this->notice .= '<h1>' . sprintf( esc_html__( 'Welcome to %1$s', 'tarja' ), $this->theme_name ) . '</h1>';
			$this->notice .= '<p>';
			$this->notice .=
				sprintf( /* Translators: Notice */
					esc_html__( 'Welcome! Thank you for choosing %3$s! To fully take advantage of the best our theme can offer please make sure you visit our %1$swelcome page%2$s.', 'tarja' ),
			...
			$this->notice .= '<p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->theme_slug . '-welcome' ) ) . '" class="button" style="text-decoration: none;"> ' . sprintf( esc_html__( 'Get started with %1$s', 'tarja' ), $this->theme_name ) . '</a></p>';
	...
	public function welcome_screen_menu() {
		...
		$title = sprintf( esc_html__( 'About %1$s', 'tarja' ), esc_html( $this->theme_name ) );
	...
	private function set_default_sections( $config = array() ) {
		$arr = array(
			'getting-started'     => array(
				...
				'label' => __( 'Getting Started', 'tarja' ),
			...
			'recommended-actions' => array(
				...
				'label' => __( 'Recommended Actions', 'tarja' ),
			...
			'recommended-plugins' => array(
				...
				'label' => __( 'Recommended Plugins', 'tarja' ),
	 ...
	 * @return string;
	 */
	public function tarja_set_pages() {
		if ( ! empty( $_GET ) ) {
			/**
			 * Check action
			 */
			if ( ! empty( $_GET['action'] ) && 'set_page_automatic' === $_GET['action'] ) {
				...
				wp_redirect( self_admin_url( 'themes.php?page=tarja-welcome&tab=' . $active_tab ) );
			}
		}
