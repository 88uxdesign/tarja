...
/**
 * Class Widget_tarja_Recent_Posts
 */
class Widget_tarja_Recent_Posts extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct(
			'tarja_Recent',
			__( 'tarja Recent Posts', 'tarja' ),
			array(
				'description'                 => esc_html__( 'Displays your most recent posts (thumbnail included for the posts).', 'tarja' ),
	...
	public function widget( $args, $instance ) {
		...
		$defaults = array(
			'title'      => esc_html__( 'Recent Posts', 'tarja' ),
			'show_title' => 'yes',
			'limit'      => 5,
			'offset'     => 0,
			'show_date'  => 'no',
		...
		$args['before_widget'] = str_replace( 'class="', 'class="tarja-recent-posts ', $args['before_widget'] );

		if ( 'no' === $instance['show_title'] ) {
			$args['before_widget'] = str_replace( 'class="tarja-recent-posts', 'class="tarja-recent-posts no-title', $args['before_widget'] );
		}
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title'      => esc_attr__( 'Recent Posts', 'tarja' ),
			'show_title' => 'yes',
			'limit'      => 5,
			'offset'     => 0,
			'show_date'  => 'no',
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_title' ) ); ?>"><?php esc_html_e( 'Show Title', 'tarja' );
				?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_date' ) ); ?>"><?php esc_html_e( 'Show Date', 'tarja' );
				?>:</label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'limit' ) ); ?>"><?php esc_html_e( 'Number of posts', 'tarja' ); ?>
				:</label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'offset' ) ); ?>"><?php esc_html_e( 'Offset', 'tarja' ); ?>
				:</label>
